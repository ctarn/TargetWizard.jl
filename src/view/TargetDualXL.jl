...
plot_lc(tg, dfs_ft, dfs_m1, dfs_m2, ε) = begin
    ...
    append!(ls, [scatter(;
        x=dfs_m2[1].rt[i:i], y=[UniMZ.error_rel(tg.mz, dfs_m2[1].mz[i])], name="A MS2#$(dfs_m2[1].id[i])",
        customdata=[(:A, i)], mode="markers", marker_size=4 * (1 + length(dfs_m2[1].psm[i])), marker_symbol="circle"
    ) for i in tg.m2_all_a_])
    append!(ls, [scatter(
        x=dfs_m2[2].rt[i:i], y=[UniMZ.error_rel(tg.mz, dfs_m2[2].mz[i])], name="B MS2#$(dfs_m2[2].id[i])",
...
plot_psm(iden, spec, ε, ele_plink, aa_plink, mod_plink, xl_plink) = begin
    ...
    ionss = UniMZ.build_ions_crosslink(spec.peaks, seqs, modss, linker, sites, ε, ele_plink, aa_plink, mod_plink)
    p_seq = UniMZ.Plotly.seq_crosslink(seqs, modss, sites, ionss)
    p_psm = UniMZ.Plotly.spec(spec.peaks, filter(i -> i.peak > 0, vcat(ionss...)))
...
process(path; path_ms, path_psm, out, path_xl, path_ft, fmt, linker, ε, fdr, cfg, host, port) = begin
    ...
    df_tg.ft_a_, df_tg.ft_b_ = map(dfs_ft) do df_ft
        ...
        mzs = map(x -> x[1], tmp)
        ids = map(x -> x[2], tmp)
        return [sort(filter(x -> df_ft[x, :z] == r.z, ids[UniMZ.argquery_ε(mzs, r.mz, ε)])) for r in eachrow(df_tg)]
    end
    ...
    df_tg.psm_all_a_, df_tg.psm_all_b_ = map(dfs_psm) do df_psm
        ...
        mzs = map(x -> x[1], tmp)
        ids = map(x -> x[2], tmp)
        return [sort(filter(x -> df_psm[x, :z] == r.z, ids[UniMZ.argquery_ε(mzs, r.mz, ε)])) for r in eachrow(df_tg)]
    end
    ...
    df_tg.m2_all_a_, df_tg.m2_all_b_ = map(dfs_m2, M2Is) do df_m2, M2I
        ...
        mzs = map(x -> x[1], tmp)
        ids = map(x -> x[2], tmp)
        return [map(x -> M2I[x], sort(ids[UniMZ.argquery_ε(mzs, r.mz, ε)])) for r in eachrow(df_tg)]
    end
...
main() = begin
    ...
    ArgParse.@add_arg_table! settings begin
        "target"
            ...
            required = true
        "--ms"
            help = "two .umz or .ms1/2 files; .ms2/1 files should be in the same directory for .ms1/2"
            required = true
            nargs = 2
        "--psm"
