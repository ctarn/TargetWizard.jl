...
prepare(args) = begin
    ...
    return (; df, out, name, ε, fdr_min, fdr_max, fdr_ge, fdr_le, td, batch_size, rt, lc, fmt)
end

process(paths; df, out, name, ε, fdr_min, fdr_max, fdr_ge, fdr_le, td, batch_size, rt, lc, fmt) = begin
    s = trues(size(df, 1))
